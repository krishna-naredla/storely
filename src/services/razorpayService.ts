...
import { PlatformRazorpayConfig, PlatformPricingPlan, PlatformPaymentTransaction } from '../types/admin';
...
export async function getRazorpayConfig(): Promise<PlatformRazorpayConfig> {
  ...
  localStorage.setItem('storelly_razorpay_config', JSON.stringify(DEFAULT_RAZORPAY_CONFIG));
  return DEFAULT_RAZORPAY_CONFIG;
}

// Save Razorpay configuration to global system_settings
export async function saveRazorpayConfig(config: PlatformRazorpayConfig): Promise<void> {
  ...
  window.dispatchEvent(new CustomEvent('storelly_razorpay_config_changed', { detail: payload }));
}
...
export async function initiateRazorpaySubscription(options: RazorpayCheckoutOptions): Promise<void> {
  ...
  const amountInPaise = Math.max(1, Math.round(plan.monthlyPrice * 100)); // Razorpay uses smallest currency unit (paise)
  ...
  const handlePaymentSuccess = async (response: {
    ...
    try {
      ...
      console.warn('Error recording payment transaction to Firestore:', err);
    }

    // Save transaction locally
    try {
      const existingRaw = localStorage.getItem('storelly_payment_transactions');
      const existing: PlatformPaymentTransaction[] = existingRaw ? JSON.parse(existingRaw) : [];
      existing.unshift(transaction);
      localStorage.setItem('storelly_payment_transactions', JSON.stringify(existing));
    } catch {}

    // Auto-upgrade business subscription if business ID is present
    if (config.autoUpgradePlan && business?.id) {
      try {
        const planSlug = plan.name.toLowerCase().includes('pro') ? 'pro' : plan.id.replace('plan_', '');
        await setDoc(
        ...
        console.warn('Error updating business subscription status in Firestore:', err);
      }
    }
    ...
    window.dispatchEvent(new CustomEvent('storelly_payment_completed', { detail: transaction }));
  ...
  // If Razorpay SDK loaded and valid key provided
  if (scriptLoaded && window.Razorpay) {
    const rzpOptions = {
      key: config.keyId || 'rzp_test_StorellyDemo369',
      ...
      prefill: {
        ...
        contact: customer?.phone || business?.phone || business?.whatsapp || '9876543210',
      },
      notes: {
        plan_id: plan.id,
        plan_name: plan.name,
    ...
    try {
      ...
      console.warn('Failed to open Razorpay instance, falling back to instant sandbox flow:', e);
    }
  }

  // Graceful Sandbox / Test Mode fallback when running in sandboxed container or offline
  const isConfirm = window.confirm(
    `[Storelly Razorpay Gateway (${config.isTestMode ? 'TEST MODE' : 'LIVE'})]\n\n` +
      `Proceed to authorize payment of ₹${plan.monthlyPrice} for "${plan.name}" subscription tier?`
  );
  ...
  if (isConfirm) {
    ...
    if (onFailure) onFailure(new Error('Cancelled by user'));
  }
}

// Fetch all payment transactions for Admin
export async function getAllPaymentTransactions(): Promise<PlatformPaymentTransaction[]> {
  try {
    ...
    if (list.length > 0) {
      ...
      localStorage.setItem('storelly_payment_transactions', JSON.stringify(list));
      return list;
    }
  } catch (err) {
