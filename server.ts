...
const CLOUDINARY_API_SECRET = process.env.CLOUDINARY_API_SECRET || "NwVtyH9n3GJDgeTXivGga1O6diY";
const TOKEN_SIGNING_SECRET = process.env.JWT_SECRET || CLOUDINARY_API_SECRET || "storelly_digital_secret_key_2026";
...
function verifyDownloadToken(token: string): {
  valid: boolean;
  expired: boolean;
  payload?: { itemId: string; orderId: string; fileUrl: string; fileName?: string; expiresAt: number };
} {
  try {
...
async function startServer() {
  ...
  app.post("/api/digital/delete-file", async (req, res) => {
    try {
      ...
      const targetType = resourceType === "video" || resourceType === "raw" ? resourceType : "image";
      ...
      res.status(500).json({ error: err.message || "Failed to delete file from Cloudinary" });
    }
  });
  ...
  app.post("/api/digital/free", async (req, res) => {
    try {
      const { itemId, orderId, fileUrl, fileName, customerName, customerPhone, merchantName = "Storelly" } = req.body;
      if (!fileUrl && !itemId) {
      ...
      const whatsAppUrl = cleanPhone ? `https://wa.me/${cleanPhone.startsWith('91') || cleanPhone.length > 10 ? cleanPhone : '91' + cleanPhone}?text=${waText}` : undefined;
      ...
      res.status(500).json({ error: err.message || "Error processing free download" });
    }
  });
  ...
  app.post("/api/digital/create-order", async (req, res) => {
    try {
      const { itemId, amount, customerName, customerPhone, currency = "INR" } = req.body;
      if (!amount || amount <= 0) {
        return res.status(400).json({ error: "Invalid product amount" });
      }

      const amountInSubunits = Math.round(Number(amount) * 100);
      const generatedOrderId = "order_digi_" + Date.now() + "_" + Math.random().toString(36).substring(2, 7);
      ...
      res.json({
        ...
        currency,
        keyId: process.env.RAZORPAY_KEY_ID || "rzp_test_dummy_creators",
      });
    } catch (err: any) {
      res.status(500).json({ error: err.message || "Failed to create digital order" });
    }
  });
  ...
  app.post("/api/digital/verify-payment", async (req, res) => {
    try {
      ...
      // In production with real Razorpay keys: verify HMAC signature
      const rzpSecret = process.env.RAZORPAY_KEY_SECRET;
      if (rzpSecret && razorpay_signature && razorpay_order_id && razorpay_payment_id) {
        const expectedSignature = crypto
      ...
      const cleanPhone = (customerPhone || "").replace(/[^0-9]/g, "");
      const formattedPhone = cleanPhone ? (cleanPhone.startsWith('91') || cleanPhone.length > 10 ? cleanPhone : '91' + cleanPhone) : '';
      ...
      const waText = encodeURIComponent(
        ...
        `🧾 *Order ID:* ${orderId}\n` +
        (razorpay_payment_id ? `💳 *Payment Ref:* ${razorpay_payment_id}\n` : '') +
      ...
      const whatsAppUrl = formattedPhone ? `https://wa.me/${formattedPhone}?text=${waText}` : undefined;
  ...
  app.post("/api/digital/whatsapp-deliver", async (req, res) => {
    try {
      ...
      if (!customerPhone && !fileUrl) {
        return res.status(400).json({ error: "Missing customer phone or file info" });
      }
      ...
      res.json({
        ...
        expiresAt,
        whatsAppUrl,
        message: "Signed download link generated and prepared for WhatsApp direct delivery",
      });
    } catch (err: any) {
      res.status(500).json({ error: err.message || "Failed to deliver WhatsApp download link" });
    }
  });
  ...
  app.post("/api/digital/resend-link", (req, res) => {
    try {
      const { orderId, itemId, fileUrl, fileName, phone, customerName = "Customer", productTitle, merchantName = "Storelly" } = req.body;
      if (!fileUrl && !itemId) {
        return res.status(400).json({ error: "Missing order or file information" });
      }
      ...
      res.status(500).json({ error: err.message || "Failed to regenerate download link" });
    }
  });
    ...
    try {
      ...
      if (fileUrl.startsWith("data:")) {
        ...
        if (matches && matches.length === 3) {
          ...
          res.setHeader("Content-Disposition", `attachment; filename="${check.payload?.fileName || "digital-asset"}"`);
  ...
  app.post("/api/events/whatsapp-ticket", (req, res) => {
    try {
      ...
      res.status(500).json({ error: err.message || "Failed to generate WhatsApp ticket link" });
    }
  });
  ...
  app.post("/api/quotes/whatsapp-quote", (req, res) => {
    try {
      ...
      res.status(500).json({ error: err.message || "Failed to prepare custom quote WhatsApp link" });
    }
  });
