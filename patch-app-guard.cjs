...
// 1. Add import
if (!code.includes('CreatorAuthGuard')) {
  code = code.replace(
...
// 2. Replace the module routing logic
const oldRoutingBlock = `      const isBioLink = pathname.startsWith('/@');
...
const newRoutingBlock = `      const isBioLink = pathname.startsWith('/@');
      const isPortfolio = pathname.startsWith('/portfolio');
...
if (code.includes(oldRoutingBlock)) {
  code = code.replace(oldRoutingBlock, newRoutingBlock);
  fs.writeFileSync('src/App.tsx', code);
  console.log('patched App.tsx with CreatorAuthGuard');
} else {
  console.log('could not find old block in App.tsx');
}
